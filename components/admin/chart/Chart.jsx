...
const Chart = ({ aspect, title, users, type }) => {
  ...
  //   var total = 0;
  //   const filtered = users.filter(user=>user.createdAt.split("-")[1]==monthStr&&user.createdAt.split("-")[0]==year);
  //   filtered.map(user=>total=total+1);
  //   return total;
  // };
  ...
  return (
    <div className={styles.chart}>
      <div className={styles.title}>{`last 6 month ${type}s statistics`}</div>
